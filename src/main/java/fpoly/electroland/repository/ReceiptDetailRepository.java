...
public interface ReceiptDetailRepository extends JpaRepository<ReceiptDetail, Integer> {
    ...
    @Query("SELECT p.name, SUM(rd.quantity) " +
            ...
            "JOIN rd.receipt r " +
            "JOIN rd.product p " +
            "WHERE r.receiptStatus.id = 5 AND r.receiptDate BETWEEN :startDate AND :endDate " +
            "GROUP BY p.name " +
    ...
    @Query("SELECT SUM(p.amount) FROM Receipt r " +
            "JOIN r.payment p " +
            "WHERE r.receiptStatus.id = 5 AND r.receiptDate >= :startDate AND r.receiptDate <= :endDate")
   ...
   @Query(value = "SELECT MONTH(r.receiptDate) AS month, COALESCE(SUM(p.amount), 0) AS value " +
               ...
               "WHERE rs.id = 5 AND YEAR(r.receiptDate) = :year " +
